...
This directory contains an EasyPanel template for Custom RAG Pipeline. It follows the
...
- Custom RAG Pipeline, exposed on port `8502`.
- SurrealDB v2, kept private on the project network and persisted in a volume.

## Testing

1. Copy this directory to `templates/rag-backend` in
...
Set an Custom RAG Pipeline password in the form before deploying. If it is left blank,
the template generates one and stores it in the app service environment as
`rag_backend_PASSWORD`.

After deployment, open the EasyPanel domain and configure AI providers from
Custom RAG Pipeline's Settings > API Keys page.
