...
```bash
for ref in lfnovo/rag_backend:<ver> lfnovo/rag_backend:<ver>-single ghcr.io/lfnovo/rag-backend:<ver> ghcr.io/lfnovo/rag-backend:<ver>-single; do
...
docker exec <that-container> /surreal export --conn http://localhost:8000 \
  --user root --pass root --ns rag_backend --db <that-db> /dev/stdout > /tmp/dev-dump.surql
...
docker rmi lfnovo/rag_backend:<ver> lfnovo/rag_backend:local 2>/dev/null
...
for n in <numbers>; do
  STATE=$(gh api "repos/lfnovo/rag-backend/issues/$n" --jq 'if .pull_request then "pr" else .state end')
