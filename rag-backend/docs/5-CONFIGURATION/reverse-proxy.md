...
Deploy Custom RAG Pipeline behind nginx, Caddy, Traefik, or other reverse proxies with custom domains and HTTPS.

---

## Simplified Setup (v1.1+)

Starting with v1.1, Custom RAG Pipeline uses Next.js rewrites to simplify configuration. **You only need to proxy to one port** - Next.js handles internal API routing automatically.
...
server {
    ...
    location / {
        proxy_pass http://rag-backend:8502;
...
notebook.example.com {
    reverse_proxy rag-backend:8502 {
        transport http {
            read_timeout 600s
            write_timeout 600s
        }
...
# See full base setup: https://github.com/lfnovo/rag-backend/blob/main/docker-compose.yml
services:
  rag-backend:
...
> **Note:** This example only shows the rag-backend and nginx services. You also need a `surrealdb` service. See the [full base docker-compose.yml](https://github.com/lfnovo/rag-backend/blob/main/docker-compose.yml) for the complete setup.
...
services:
  rag-backend:
    ...
    environment:
      - API_URL=https://notebook.example.com
      - rag_backend_ENCRYPTION_KEY=${rag_backend_ENCRYPTION_KEY}
      - rag_backend_PASSWORD=${rag_backend_PASSWORD}
    volumes:
...
location /api/ {
    proxy_pass http://rag-backend:5055/api/;
    proxy_http_version 1.1;
    proxy_set_header Host $host;
    proxy_set_header X-Real-IP $remote_addr;
    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
...
Accessing Custom RAG Pipeline from a different machine on your network:

**Step 1: Get your server IP**
```bash
# On the server running Custom RAG Pipeline:
hostname -I
# or
ifconfig | grep "inet "
...
server {
    ...
    location / {
        proxy_pass http://rag-backend:5055;
        proxy_http_version 1.1;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
...
services:
  frontend:
    image: lfnovo/rag_backend_frontend:v1-latest
    pull_policy: always
    environment:
      - API_URL=https://notebook.example.com
    ports:
  ...
  api:
    image: lfnovo/rag_backend_api:v1-latest
    pull_policy: always
    environment:
      - rag_backend_ENCRYPTION_KEY=${rag_backend_ENCRYPTION_KEY}
    ports:
...
1. **Check API_URL is set**:
   ```bash
   docker exec rag-backend env | grep API_URL
   ```
...
2. Check container logs: `docker logs rag-backend`
...
**Cause:** Your reverse proxy has a default timeout (often 30s) that's shorter than Custom RAG Pipeline's operations.
...
**Version Requirement:**
- **Custom RAG Pipeline v1.3.2+** is required for file uploads >10MB
...
**Note:** Custom RAG Pipeline's API includes CORS headers in error responses, but this only works for errors that reach the application. Proxy-level errors (like 413 from nginx) need to be configured at the proxy level.
...
3. **Reverse proxy not forwarding `/api/*` correctly**:
   ...
   location /api/ {
       proxy_pass http://rag-backend:5055/api/;  # Note the trailing slash!
   }
   ```
...
This happens when:
- You have set `rag_backend_PASSWORD` for authentication
...
9. **Monitor logs** regularly: `docker logs rag-backend`
...
If you're running Custom RAG Pipeline **version 1.0.x or earlier**, you may need to use the legacy two-port configuration where you explicitly route `/api/*` to port 5055.

**Check your version:**
```bash
docker exec rag-backend cat /app/package.json | grep version
```
