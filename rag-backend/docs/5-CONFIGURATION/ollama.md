...
Ollama provides free, local AI models that run on your own hardware. This guide covers everything you need to know about setting up Ollama with Custom RAG Pipeline, including different deployment scenarios and network configurations.
...
### 3. Configure Custom RAG Pipeline
...
> **Deprecated**: `OLLAMA_API_BASE` is a deprecated fallback. It still works today, but there is no guarantee it keeps working in future releases, and new automation should not be built on it. Configure Ollama via **Manage → Models** instead. For headless/CI/Docker setups, see the [environment reference](environment-reference.md#legacy-ai-provider-environment-variables-deprecated) and the provisioning discussion in [#765](https://github.com/lfnovo/rag-backend/discussions/765).
...
When both Custom RAG Pipeline and Ollama run directly on your machine:
...
### Scenario 2: Custom RAG Pipeline in Docker, Ollama on Host

When Custom RAG Pipeline runs in Docker but Ollama runs on your host machine:
...
When both Custom RAG Pipeline and Ollama run in the same Docker Compose stack:
...
# https://github.com/lfnovo/rag-backend/blob/main/docker-compose.yml
services:
  rag-backend:
    image: lfnovo/rag_backend:v1-latest
    pull_policy: always
...
This is the most common cause of "Failed to send message" errors. Custom RAG Pipeline requires the **exact model name** as it appears in Ollama.
...
**Step 2: Use the exact name when adding the model in Custom RAG Pipeline**
...
**Note:** Some models use `:latest` as the default tag. If `ollama list` shows `model:latest`, you must use `model:latest` in Custom RAG Pipeline, not just `model`.

**Step 3: Configure in Custom RAG Pipeline**
...
**1. "Ollama unavailable" in Custom RAG Pipeline**
...
```bash
# If Custom RAG Pipeline runs in Docker or on a different machine,
# Ollama must bind to all interfaces, not just localhost
export OLLAMA_HOST=0.0.0.0:11434
ollama serve
```
> **Why this is needed:** By default, Ollama only accepts connections from `localhost` (127.0.0.1). When Custom RAG Pipeline runs in Docker or on a different machine, it can't reach Ollama unless you configure `OLLAMA_HOST=0.0.0.0:11434` to accept external connections.
...
**From inside Custom RAG Pipeline container, test Ollama:**
```bash
# Get into container
...
1. **Model name mismatch**: The model name in Custom RAG Pipeline doesn't exactly match `ollama list`
2. **No default model configured**: You haven't set a default chat model in Manage → Models
3. **Model was deleted**: You removed the model from Ollama but didn't update Custom RAG Pipeline's defaults
4. **Model record deleted**: The model was removed from Custom RAG Pipeline but is still set as default
...
# Compare with what's configured in Custom RAG Pipeline
...
1. Refresh the Custom RAG Pipeline page
...
docker run --network host lfnovo/rag_backend:v1-latest  # for quick testing only
```
...
# Custom RAG Pipeline configuration (configure via Manage → Models instead)
...
- [Custom RAG Pipeline Discord](https://discord.gg/37XJPXfz2w) - Integration help
...
This comprehensive guide should help you successfully deploy and optimize Ollama with Custom RAG Pipeline. Start with the Quick Start section and refer to specific scenarios as needed.
