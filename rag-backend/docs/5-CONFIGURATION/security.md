...
Protect your Custom RAG Pipeline deployment with password authentication and production hardening.

---

## API Key Encryption

Custom RAG Pipeline encrypts API keys stored in the database using Fernet symmetric encryption (AES-128-CBC with HMAC-SHA256).
...
```bash
# .env or docker.env
rag_backend_ENCRYPTION_KEY=my-secret-passphrase
```
...
|---------|---------|----------------|
| Password | None - auth is fully disabled until `rag_backend_PASSWORD` is set | Development only |
| Encryption Key | **None** (must be configured) | Required for API key storage |

**The encryption key has no default.** You must set `rag_backend_ENCRYPTION_KEY` before using the API key configuration feature. Without it, encrypting/decrypting API keys will fail.
...
```yaml
environment:
  - rag_backend_PASSWORD_FILE=/run/secrets/app_password
  - rag_backend_ENCRYPTION_KEY_FILE=/run/secrets/encryption_key
```
...
services:
  rag_backend:
    ...
    pull_policy: always
    environment:
      - rag_backend_ENCRYPTION_KEY=your-secret-encryption-key
      - rag_backend_PASSWORD=your_secure_password
    # ... rest of config
```
...
rag_backend_PASSWORD=MySecure2024!Research#Tool
rag_backend_PASSWORD=Notebook$Dev$2024$Strong!

# Generated (recommended)
rag_backend_PASSWORD=$(openssl rand -base64 24)
```
...
```bash
# DON'T use these
rag_backend_PASSWORD=password123
rag_backend_PASSWORD=CustomRAGPipeline
rag_backend_PASSWORD=admin
```
...
# Usage
client = CustomRAGPipelineClient("http://localhost:5055", "your_password")
...
Custom RAG Pipeline's password protection provides **basic access control**, not enterprise-grade security:
...
docker exec rag-backend env | grep rag_backend_PASSWORD

# Check logs
docker logs rag-backend | grep -i auth
...
echo "Password length: $(echo -n $rag_backend_PASSWORD | wc -c)"
```
