...
RUST_LOG=rag_backend::database=debug
```
...
LANGCHAIN_PROJECT="Custom RAG Pipeline"
```
...
```env
# Protect your Custom RAG Pipeline instance
...
Restrict access to your Custom RAG Pipeline:
...
Custom RAG Pipeline uses multiple engines for content extraction. Which one runs is chosen in **Settings → Content Processing** (see the user-guide page on [Content Processing Engines](../3-USER-GUIDE/content-processing-engines.md)); the variables below configure them.
...
Renders JavaScript pages in a local Chromium browser — no API key required. Crawl4AI is **optional**: enable it with `rag_backend_ENABLE_CRAWL4AI=true` and it installs on first startup (the Chromium download is cached on your data volume). To offload rendering to a remote Crawl4AI server instead — no local install needed:
...
```env
rag_backend_ENCRYPTION_KEY    # Required for storing credentials
```
...
env | grep rag_backend_ENCRYPTION_KEY
...
# Create backup
tar -czf "$BACKUP_DIR/rag-backend-$DATE.tar.gz" \
  /path/to/notebook_data \
  /path/to/surreal_data

# Keep only last 7 days
find "$BACKUP_DIR" -name "rag-backend-*.tar.gz" -mtime +7 -delete

echo "Backup complete: rag-backend-$DATE.tar.gz"
```

Add to cron:
```bash
# Daily backup at 2 AM
0 2 * * * /path/to/backup.sh >> /var/log/rag-backend-backup.log 2>&1
```
...
# On source server
docker compose down
tar -czf rag-backend-migration.tar.gz notebook_data/ surreal_data/

# Transfer to new server
scp rag-backend-migration.tar.gz user@newserver:/path/

# On new server
tar -xzf rag-backend-migration.tar.gz
docker compose up -d
```
