...
Use any server that implements the OpenAI API format with Custom RAG Pipeline. This includes LM Studio, Text Generation WebUI, vLLM, and many others.
...
Custom RAG Pipeline can connect to any server using this format.
...
### Step 3: Add Model in Custom RAG Pipeline
...
When Custom RAG Pipeline runs in Docker and your compatible server runs on the host, use the appropriate base URL when adding your credential in **Manage → Models**:
...
services:
  vllm:
    ...
    ports:
      ...
      # 8001 because SurrealDB already publishes 8000. Custom RAG Pipeline reaches
...
## Adding Models in Custom RAG Pipeline
...
```bash
docker exec -it rag-backend curl http://host.docker.internal:1234/v1/models
```
...
4. Update model name in Custom RAG Pipeline
```
