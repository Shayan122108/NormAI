...
Custom RAG Pipeline uses a **credential-based system** for managing AI providers:
...
> **Prerequisite**: You must set `rag_backend_ENCRYPTION_KEY` in your docker-compose.yml before storing credentials. See [API Configuration](../3-USER-GUIDE/api-configuration.md#encryption-setup) for details.
...
**Configure in Custom RAG Pipeline:**
...
**Available Models (in Custom RAG Pipeline):**
...
- Reranking is not yet available in Custom RAG Pipeline (tracked separately).
...
4. Base URL defaults to `http://localhost:11435/v1` (use `http://host.docker.internal:11435/v1` if Custom RAG Pipeline is in Docker)
...
By default, Custom RAG Pipeline uses the LLM provider's embeddings. Embedding models are discovered and registered through the same credential system — when you discover models from a credential, embedding models are included alongside language models.
...
3. **Set `rag_backend_ENCRYPTION_KEY`** in your docker-compose.yml (required for storing credentials)
