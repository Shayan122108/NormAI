...
Run Custom RAG Pipeline with a **separately installed Ollama** (not via Docker). This avoids Docker running the Ollama service while you use your own local Ollama installation.

## Prerequisites

1. **Docker Desktop** installed (for SurrealDB and Custom RAG Pipeline)
...
Create a new folder `rag-backend-external-ollama` and add these files:
...
## Step 3: Connect Custom RAG Pipeline to Host Ollama (1 min)

When Custom RAG Pipeline runs inside Docker, it cannot reach `localhost:11434` on your host directly. Use the special hostname:

| Host OS | Ollama URL in Custom RAG Pipeline |
|---------|----------------------------|
...
## Step 4: Start Custom RAG Pipeline (1 min)

Open terminal in your `rag-backend-external-ollama` folder:
...
## Step 7: Access Custom RAG Pipeline (instant)
...
3. For Windows/macOS, ensure `host.docker.internal` is reachable from inside the container:
   ```bash
   docker exec <rag_backend_container> curl http://host.docker.internal:11434/api/version
   ```
...
- **Add more models**: Run `ollama pull <model>`, then re-discover from Custom RAG Pipeline
