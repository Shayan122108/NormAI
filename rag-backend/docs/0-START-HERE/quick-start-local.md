...
Get Custom RAG Pipeline running with **100% local AI** using Ollama. No cloud API keys needed, completely private.
...
Create a new folder `rag-backend-local` and add this file:
...
Open terminal in your `rag-backend-local` folder:
...
docker exec rag-backend-local-ollama-1 ollama pull neural-chat

# OR: Even better quality, more VRAM needed
docker exec rag-backend-local-ollama-1 ollama pull llama2
```
...
## Step 5: Access Custom RAG Pipeline (instant)
...
```bash
# Show available GPUs
docker exec rag-backend-local-ollama-1 ollama ps
...
docker exec rag-backend-local-ollama-1 ollama list
...
4. In Custom RAG Pipeline, go to **Manage** → **Models**
