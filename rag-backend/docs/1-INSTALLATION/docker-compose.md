...
> **Alternative Registry:** All images are available on both Docker Hub (`lfnovo/rag_backend`) and GitHub Container Registry (`ghcr.io/lfnovo/rag-backend`). Use GHCR if Docker Hub is blocked or you prefer GitHub-native workflows.
...
The official `docker-compose.yml` is in the root of our repository: [View on GitHub](https://github.com/lfnovo/rag-backend/blob/main/docker-compose.yml)
...
- (Optional) To use database credentials other than the default `root:root`, create a `.env` file next to `docker-compose.yml` with `SURREAL_USER=...` and `SURREAL_PASSWORD=...` — both services pick them up automatically ([.env.example](https://github.com/lfnovo/rag-backend/blob/main/.env.example) shows the full format)

---

## Step 2: Start Services (2 min)

Open terminal in the `rag-backend` folder:
...
✅ rag_backend running on :8502 (UI) and :5055 (API)
```
...
You should see the Custom RAG Pipeline interface!
...
Done! You now have a fully working Custom RAG Pipeline instance.
...
curl -o docker-compose.yml https://raw.githubusercontent.com/lfnovo/rag-backend/main/examples/docker-compose-ollama.yml
...
docker exec rag-backend-local-ollama-1 ollama pull mistral
```
...
|----------|---------|---------|
| `rag_backend_ENCRYPTION_KEY` | Encryption key for credentials | `my-secret-key` |
...
| `SURREAL_NAMESPACE` | Database namespace | `rag_backend` |
| `SURREAL_DATABASE` | Database name | `rag_backend` |
| `API_URL` | API external URL | `http://localhost:5055` |
| `rag_backend_EMBEDDING_BATCH_SIZE` | Override embedding batch size for stricter/local providers (recommended: `8` for CPU-only local setups) | `50` |
...
- **Issues**: [GitHub Issues](https://github.com/lfnovo/rag-backend/issues)
