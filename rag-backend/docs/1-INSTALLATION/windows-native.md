# Custom RAG Pipeline Windows Installation Guide (Native, No Docker)

This guide documents how to install and run [Custom RAG Pipeline](https://github.com/lfnovo/rag-backend) on Windows **natively without Docker or WSL**.
...
1. **Clone and setup:**
   ...
   git clone https://github.com/lfnovo/rag-backend.git
   cd rag-backend
   uv sync
...
3. **Start the four services**, each in its own terminal, from the `rag-backend` folder.

   > Custom RAG Pipeline does not ship a launcher script — start the services manually as below (or wrap them in your own `.bat`, see [Optional: one-click launcher](#optional-one-click-launcher)).

   ```batch
   REM Optional: point Custom RAG Pipeline at a separate data folder (see Issue 4 below).
   REM Set this in each terminal before running, or skip to use ./data.
   set DATA_FOLDER=%USERPROFILE%\Projects\rag-backend-data
...
```
YourProjectsFolder\
├── rag-backend\           # Source code (git clone)
│   ├── .venv\               # Python virtual environment (created by uv)
│   ├── frontend\            # Next.js frontend
│   ├── commands\            # Worker command modules
│   └── .env                 # Your configuration
├── rag-backend-data\      # Data storage (SEPARATE from code!)
│   ├── surrealdb\           # Database files
│   ├── uploads\             # Uploaded documents
│   └── sqlite-db\           # LangGraph checkpoints
└── start-rag-backend.bat  # Optional launcher you create yourself (see below)
```
...
Custom RAG Pipeline does not ship a launcher, but you can save the following as
`start-rag-backend.bat` (anywhere you like) to start all four services with a
double-click. Adjust `ROOT` and `DATA_ROOT` to match your setup.

```batch
@echo off
REM --- adjust these two paths ---
set ROOT=%USERPROFILE%\Projects\rag-backend
set DATA_ROOT=%USERPROFILE%\Projects\rag-backend-data
...
```batch
set DATA_FOLDER=C:\path\to\rag-backend-data
```

## Configuration Files

### Modifying `rag_backend/config.py`
...
*Tested on Windows 11 ARM64 with Custom RAG Pipeline v1.6.0*
