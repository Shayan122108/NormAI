...
This document provides guidelines for writing tests in Custom RAG Pipeline. Testing is critical to maintaining code quality and preventing regressions.
...
from rag_backend.domain.notebook import Notebook
...
- Use `--cov` flag to check coverage: `uv run pytest --cov=rag_backend`
