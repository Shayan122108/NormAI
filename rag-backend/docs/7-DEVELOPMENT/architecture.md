...
Custom RAG Pipeline follows a three-tier architecture with clear separation of concerns:
...
- **Migrations**: `.surrealql` files in `rag_backend/database/migrations/`, registered in `AsyncMigrationManager` (auto-run on API startup)
...
LangGraph is a state machine library that orchestrates multi-step AI workflows. Custom RAG Pipeline uses five core workflows:

### 1. **Source Processing Workflow** (`rag_backend/graphs/source.py`)
...
### 2. **Chat Workflow** (`rag_backend/graphs/chat.py`)
...
### 3. **Ask Workflow** (`rag_backend/graphs/ask.py`)
...
### 4. **Transformation Workflow** (`rag_backend/graphs/transformation.py`)
...
### 5. **Prompt Workflow** (`rag_backend/graphs/prompt.py`)
...
Located in `rag_backend/ai/models.py`, ModelManager handles:
...
**Usage**:
```python
from rag_backend.ai.provision import provision_langchain_model
...
**Domain Objects** (`rag_backend/domain/`):
...
**Repository Pattern**:
- Database access layer (`rag_backend/database/repository.py`)
...
- Located in `rag_backend/database/migrations/`
...
1. Create `rag_backend/graphs/workflow_name.py`
...
1. Create model in `rag_backend/domain/model_name.py`
...
5. Create database migration in `rag_backend/database/migrations/` (and register it in `AsyncMigrationManager`)
