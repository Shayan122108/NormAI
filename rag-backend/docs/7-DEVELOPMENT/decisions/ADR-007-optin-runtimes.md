...
Baking both into the default image would push it into the multi-GB range and contradict Custom RAG Pipeline's lean, privacy-first, self-hostable posture (ADR-002 keeps this repo focused on the knowledge layer, delegating extraction to Content Core). Most users need neither runtime.
...
- Two booleans gate them: `rag_backend_ENABLE_DOCLING` and `rag_backend_ENABLE_CRAWL4AI`. A remote Crawl4AI server (`CRAWL4AI_API_URL`) needs no local install.
