...
> **Credential System**: Custom RAG Pipeline uses encrypted credentials stored in the database. Each credential connects to a provider and allows you to discover, register, and test models.

---

## Overview

Custom RAG Pipeline manages AI provider access through a **credential-based system**:
...
Add `rag_backend_ENCRYPTION_KEY` to your docker-compose.yml:
...
3. The models are now available throughout Custom RAG Pipeline
...
- Keep `rag_backend_ENCRYPTION_KEY` — it's still required
...
|---------|---------------|---------------------------|
| Password | None - auth is fully disabled until set | Set `rag_backend_PASSWORD` |
| Encryption Key | None (must be set) | Set `rag_backend_ENCRYPTION_KEY` to any secret string |
...
| Error on save | Encryption key not set | Set `rag_backend_ENCRYPTION_KEY` in docker-compose.yml |
